...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.height < rect2.left || rect2.height < rect1.left) {
    return false;
  }
  if (rect1.width < rect2.top || rect2.width < rect1.top) {
    return false;
  }
  return true;
}
...
function isInsideCircle(circle, point) {
  ...
  const sqrRad = circle.radius ** 2;

  if (coordX + coordY < sqrRad) {
    return true;
  }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let result = '';
  if (isEndIncluded && isEndIncluded) {
    if (a < b) {
      result = `[${a}, ${b}]`;
    } else {
      result = `[${b}, ${a}]`;
...
function reverseInteger(num) {
  ...
  const newNum = Number(numToStr.reverse().join(''));
  return newNum;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const minLength = Math.min(...splitPathes.map((components) => components.length));
  const newArr = [];
  for (let i = 0; i < minLength; i += 1) {
    if (splitPathes.every((item) => item[i] === splitPathes[0][i])) {
      newArr.push(splitPathes[0][i]);
    } else {
      break;
    }
  }
